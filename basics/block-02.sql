...
SELECT
  ...
  Country
FROM Customer
WHERE Country IN ('Brazil', 'USA', 'Germany');
...
SELECT TrackId, Name, Milliseconds from Track WHERE Milliseconds > 250000;
...
FROM Invoice
WHERE InvoiceDate BETWEEN '2020-01-01' AND '2022-12-31';

-- 10. Condiciones múltiples
-- Trae todos los clientes que:
-- 	•	Sean de Brazil, AND
-- 	•	Su nombre comience con la letra J.
SELECT CustomerId, FirstName, LastName, Country from Customer WHERE Country = 'Brazil' AND FirstName LIKE 'J%';
